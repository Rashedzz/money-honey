...
import { ForecastAccuracyView } from '../stock/ForecastAccuracyView';
import {
...
export const StockMarketScreen: React.FC<StockMarketScreenProps> = ({
  ...
  const handleShareReport = () => {
    ...
    const shareText = `📊 BANGLADESH AI RESEARCH REPORT: ${selectedStock.companyName} (${selectedStock.symbol})\n` +
      `Recommendation: ${selectedStock.recommendation}\n` +
      `AI Score: ${selectedStock.totalAiScore}/100\n` +
      `Current LTP: ৳${selectedStock.ltp}\n` +
      `Consensus Target: ৳${selectedStock.ensembleTargetPrice} (+${selectedStock.potentialUpsidePercent}%)\n` +
      `DCF Fair Value: ৳${selectedStock.dcfIntrinsicValue} (${selectedStock.marginOfSafetyPercent}% Margin of Safety)\n` +
      `Stop-Loss: ৳${selectedStock.supportLevel}\n` +
      `Generated by Money-Honey Bangladesh AI Investor.`;

    if (typeof navigator !== 'undefined' && navigator.share) {
      navigator.share({
        title: `${selectedStock.symbol} - AI Investment Report`,
        text: shareText,
      }).catch(() => {});
    } else if (typeof navigator !== 'undefined' && navigator.clipboard) {
      navigator.clipboard.writeText(shareText);
      Alert.alert('Report Copied', 'Full AI stock research summary copied to clipboard! You can share it anywhere.');
    }
  };
            ...
            {selectedStock && (
              <>
                <View style={styles.modalHeader}>
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                    ...
                    >
                      ...
                      <Text style={styles.headerActionText}>Print / PDF</Text>
                    </TouchableOpacity>
                    ...
                    >
                      ...
                      <Text style={[styles.headerActionText, { color: '#16A34A' }]}>Share</Text>
                    </TouchableOpacity>
                ...
                >
                  ...
                  <View style={[styles.thesisBox, { backgroundColor: '#F0FDF4', borderColor: '#BBF7D0', marginBottom: 12, padding: 14 }]}>
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                        <Text style={{ fontSize: 20 }}>🎯</Text>
                        <View>
                          <Text style={{ fontSize: 14, fontWeight: '900', color: selectedStock.recommendation.includes('BUY') ? '#16A34A' : '#DC2626' }}>
                            SPECIFIC ACTION: {selectedStock.recommendation} (CONFIDENCE: {selectedStock.totalAiScore}%)
                          </Text>
                          <Text style={{ fontSize: 11, color: '#166534' }}>
                            {selectedStock.recommendation.includes('BUY') ? 'Optimal asymmetric risk-reward setup. Follow execution limits strictly.' : 'Unfavorable risk-reward. Exit rallies or avoid fresh entry.'}
                          </Text>
                        </View>
                      </View>
                      <View style={{ flexDirection: 'row', gap: 6 }}>
                        <View style={{ backgroundColor: '#DCFCE7', paddingHorizontal: 10, paddingVertical: 4, borderRadius: Radius.sm }}>
                          <Text style={{ fontSize: 11, fontWeight: '900', color: '#16A34A' }}>BUY ZONE: ৳{Math.round((selectedStock.ltp * 0.97) * 10) / 10} - ৳{selectedStock.ltp}</Text>
                        </View>
                        <View style={{ backgroundColor: '#FEE2E2', paddingHorizontal: 10, paddingVertical: 4, borderRadius: Radius.sm }}>
                          <Text style={{ fontSize: 11, fontWeight: '900', color: '#EF4444' }}>STOP-LOSS: ৳{selectedStock.supportLevel}</Text>
                        </View>
                      </View>
                    </View>

                    {/* Specific Targets & Rules Grid */}
                    <View style={[styles.kpiGrid, { marginTop: 10 }]}>
                      <View style={[styles.kpiItem, { backgroundColor: '#FFFFFF' }]}>
                        <Text style={styles.kpiLabel}>TARGET 1 (3-6 MO)</Text>
                        <Text style={[styles.kpiVal, { color: '#16A34A', fontSize: 15 }]}>৳{Math.round((selectedStock.ltp * 1.12) * 10) / 10} (+12%)</Text>
                        <Text style={{ fontSize: 9, color: '#64748B' }}>Conservative 1st Lock</Text>
                      </View>
                      <View style={[styles.kpiItem, { backgroundColor: '#FFFFFF' }]}>
                        <Text style={styles.kpiLabel}>TARGET 2 (6-12 MO)</Text>
                        <Text style={[styles.kpiVal, { color: '#16A34A', fontSize: 15 }]}>৳{selectedStock.ensembleTargetPrice} (+{selectedStock.potentialUpsidePercent}%)</Text>
                        <Text style={{ fontSize: 9, color: '#64748B' }}>Consensus AI Target</Text>
                      </View>
                      <View style={[styles.kpiItem, { backgroundColor: '#FFFFFF' }]}>
                        <Text style={styles.kpiLabel}>DCF VALUE TARGET</Text>
                        <Text style={[styles.kpiVal, { color: '#0284C7', fontSize: 15 }]}>৳{selectedStock.dcfIntrinsicValue} (+{selectedStock.marginOfSafetyPercent}% )</Text>
                        <Text style={{ fontSize: 9, color: '#64748B' }}>Intrinsic Fair Value</Text>
                      </View>
                      <View style={[styles.kpiItem, { backgroundColor: '#FFFFFF' }]}>
                        <Text style={styles.kpiLabel}>RISK / REWARD</Text>
                        <Text style={[styles.kpiVal, { color: '#0F172A', fontSize: 15 }]}>1 : 3.5 Ratio</Text>
                        <Text style={{ fontSize: 9, color: '#16A34A' }}>Institutional Grade</Text>
                      </View>
                      <View style={[styles.kpiItem, { backgroundColor: '#FFFFFF' }]}>
                        <Text style={styles.kpiLabel}>POSITION SIZING</Text>
                        <Text style={[styles.kpiVal, { color: '#0F172A', fontSize: 15 }]}>Max 10% - 15%</Text>
                        <Text style={{ fontSize: 9, color: '#64748B' }}>Risk Portfolio Cap</Text>
                      </View>
                    </View>

                    {/* Exit Rules */}
                    <Text style={{ fontSize: 11, color: '#334155', marginTop: 8, lineHeight: 16 }}>
                      📌 <Text style={{ fontWeight: '800' }}>Specific Execution Rule:</Text> Enter in the buy zone between ৳{Math.round((selectedStock.ltp * 0.97) * 10) / 10} and ৳{selectedStock.ltp}. Take partial 50% profit at Target 1 (৳{Math.round((selectedStock.ltp * 1.12) * 10) / 10}) and trail stop-loss to entry price. Immediately cut position if daily close breaks below ৳{selectedStock.supportLevel}.
                    </Text>
                  </View>
                  ...
                  <View style={{ flexDirection: 'row', gap: 6, flexWrap: 'wrap', marginTop: 8, marginBottom: 12, borderBottomWidth: 1, borderBottomColor: '#E2E8F0', paddingBottom: 8 }}>
                    {[
                      ...
                      { id: 'forecast_accuracy', label: '🎯 Forecast vs Actual Accuracy' },
                  ...
                  {/* TAB: AI Forecast & Recommendation Accuracy Verification */}
                  {modalSubTab === 'forecast_accuracy' && (
